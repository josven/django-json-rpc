...
def validate_params(method, D):
    if type(D['params']) == types.Object:
        keys = method.json_arg_types.keys()
        if len(keys) != len(D['params']):
            raise exceptions.InvalidParamsError('Not enough params provided for %s' % method.json_sig)
        for k in keys:
            if k not in D['params']:
                raise exceptions.InvalidParamsError('%s is not a valid parameter for %s' % (k, method.json_sig))
            if not types.Any.kind(D['params'][k]) == method.json_arg_types[k]:
                raise exceptions.InvalidParamsError('%s is not the correct type %s for %s' % (type(D['params'][k]), method.json_arg_types[k], method.json_sig))
    elif type(D['params']) == types.Array:
        arg_types = method.json_arg_types.values()
        try:
            for i, arg in enumerate(D['params']):
                if not types.Any.kind(arg) == arg_types[i]:
                    raise exceptions.InvalidParamsError('%s is not the correct type %s for %s' % (type(arg), arg_types[i], method.json_sig))
        except IndexError:
            raise exceptions.InvalidParamsError('Too many params provided for %s' % method.json_sig)
        else:
...
class JSONRPCSite(object):
    ...
    def build_response(self, version, method, request, params):
        if version == '2.0':
            if type(params) is dict:
                return method(request, **encode_kw(params))
            else:
                return method(request, *params)

        if version == '1.1':
            return method(request, *encode_arg11(params), **encode_kw(encode_kw11(params)))
    ...
    def response_dict(self, request, D, is_batch=False, version_hint='1.0', json_encoder=None):
        ...
        try:
            # params: An types.Array or types.Object, that holds the actual parameter values
            # for the invocation of the procedure. Can be omitted if empty.
            if 'params' not in D:
                D['params'] = []
            if 'method' not in D or 'params' not in D:
                raise exceptions.InvalidParamsError('Request requires str:"method" and list:"params"')
            if D['method'] not in self.urls:
                raise exceptions.MethodNotFoundError('Method not found. Available methods: %s' % ('\n'.join(self.urls.keys())))

            if 'jsonrpc' in D:
                if str(D['jsonrpc']) not in apply_version:
                    raise exceptions.InvalidRequestError('JSON-RPC version %s not supported.' % D['jsonrpc'])
                version = request.jsonrpc_version = response['jsonrpc'] = str(D['jsonrpc'])
            elif 'version' in D:
                if str(D['version']) not in apply_version:
                    raise exceptions.InvalidRequestError('JSON-RPC version %s not supported.' % D['version'])
            ...
            if not sum(map(lambda e: isinstance(R, e), (dict, str, unicode, int, long, list, set, NoneType, bool))):
                try:
        ...
        except (exceptions.InvalidCredentialsError, exceptions.Error) as e:
            ...
            signals.got_request_exception.send(sender=self.__class__, request=request)
            other_error = exceptions.OtherError(e)
    ...
    def procedure_desc(self, key):
        ...
        return {
            ...
            'params': [{'type': str(types.Any.kind(t)), 'name': k} for k, t in M.json_arg_types.iteritems()],
    ...
    def service_desc(self):
        return {
            ...
            'procs': [self.procedure_desc(k) for k in self.urls.iterkeys() if self.urls[k] != self.describe]}
